...
2. **Read architecture**: Read the target project's `book.py` to understand how blocks are wired. For reference, see `tests/test_project/book.py` or `documentation/template_project/book.py`.
...
5. **Create the block file** in `[project]/blocks/` with:
   ...
   - A `build()` function implementing the described content using `sx.*` functions
   - Proper TOC entries if the block has headings
6. **Update `blocks/__init__.py`**: Add the new module to the dynamic import list if it uses explicit imports.
...
- Use `sx.*` functions, never raw `st.*` for content
