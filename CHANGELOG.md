...
- **Native overlay slot in `st_image` — `MediaOverlay`** — a badge
  ("pastille") rendered by `st_image` itself inside the image's final
  display box, e.g. an AI-transparency mark ("✦ AI"):

  ```python
  st_image(uri="hero.png", overlay=MediaOverlay(
      text="✦ AI", position="bottom-right",
      aria_label="This image was generated by an AI model",
  ))
  ```

  - `MediaOverlay(text, position="bottom-right", css="", aria_label="")`
    (new module `streamtex/media_overlay.py`, exported from the package
    root). Positions: `bottom-right`, `top-right`, `bottom-left`,
    `top-left`. `css` is appended after the neutral default pill so
    project conventions win in cascade order; `aria_label` emits the
    accessible disclosure (art. 50 AI Act). `text` and `aria_label` are
    HTML-escaped.
  - The emission wraps the `<img>` in a `<span class="stx-media-box">`
    (`position:relative`) holding the badge
    `<span class="stx-media-overlay" role="note">`. The wrapper is
    emitted **after** the managed display settings (`display_zoom`,
    `display_width`) are resolved and carries the resolved width, so the
    badge anchors on the image's real box whatever zoom or width —
    downstream projects no longer need to recompute offsets from the
    sidecar. Auto margins (centering) found in the caller style are
    transferred onto the wrapper.
  - The badge is `pointer-events:none` (a `link=` stays fully
    clickable — the anchor wraps the whole box; the hover class stays on
    the anchor), travels inside the image element in the HTML export,
    and in editor mode wraps only the `<img>` (the "Edit Image" panel
    stays outside).
  - `overlay=None` (default) keeps the emitted HTML **byte-identical**
    to 0.7.22 — guarded by a character-exact non-regression test.
  - New chapter "Media Overlays" in the advanced manual
    (`streamtex-docs/manuals/stx_manual_advanced`) with the
    AI-transparency mark example.

### Fixed

- **`stx publish check` false negative on `__version__`** — the check only
  accepted a top-level literal `__version__ = "x.y.z"` line, so the
  library's own dynamic resolution
  (`__version__ = _pkg_version("streamtex")`, indented inside the import
  guard) failed the check and blocked `stx publish pypi`. Dynamic
  `importlib.metadata` resolution is now recognized and passes (it matches
