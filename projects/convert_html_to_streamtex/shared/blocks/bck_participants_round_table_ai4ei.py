...
def build():
    ...
    with st_grid(cols=2, grid_style=s.project.pres.grids.gap) as g:
        with g.cell():
            st_write(s.project.pres.tables.cell, "First name", "Motivations for participating to ", (s.project.colors.burnt_orange + s.bold, "AI"), (s.project.colors.forest_green + s.bold, "4"), (s.project.colors.dark_purple + s.bold, "EI "))
        with g.cell():
