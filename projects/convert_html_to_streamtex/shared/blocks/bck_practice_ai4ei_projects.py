...
def build():
    ...
    with st_list(list_type=lt.unordered, li_style=s.project.pres.lists.item) as lst:
        ...
        with lst.item():
            st_write("Produce a Generative Agent specialist of a specialized field  related to AI for Entrepreneurship & Innovation: ")
    st_space(size=3)
    with st_list(list_type=lt.unordered, li_style=s.project.pres.lists.item) as lst:
        with lst.item():
            st_write(s.project.colors.deep_red + s.bold, "AI as a Tool for E&I ")
        with lst.item():
            st_write(s.project.colors.burnt_orange + s.bold, "AI-Powered Product for E&I ")
        with lst.item():
            pass
        with lst.item():
    ...
    with st_list(list_type=lt.unordered, li_style=s.project.pres.lists.item) as lst:
        with lst.item():
            st_write("Content for each student1 slide introducing the individual tasks and its goal wrt the group project goal3 to 5 slides presenting the main outcomes of the tasks done. ")
        with lst.item():
    ...
    st_space(size=2)
    st_write(s.project.pres.paragraphs.p_xl, "4 ECTS = 4 * 30 hours = 120 hours / student ")
    st_write(s.project.pres.paragraphs.p_xl, "26 h = 8 * 3h15 -> lectures ")
    st_write(s.project.pres.paragraphs.p_xl, "10h =  5 x 2h -> lectures notes reading and exercices ")
    st_write(s.project.pres.paragraphs.p_xl + s.project.colors.bright_red + s.bold, "=> ")
    st_write(s.project.pres.paragraphs.p_xl, "+/- 80 hours / student / project ")
    st_write(s.project.pres.paragraphs.p_xl, "Report writing = +/- 40% = 30h / student / project ")
    st_space(size=3)
    st_write(
        s.project.pres.paragraphs.p_xl,
        "-> ",
        (s.project.colors.bright_red + s.bold, "4 hours per week average per student over 20 weeks "),
    )
    st_space(size=3)
    st_space(size=3)
    st_space(size=3)
    st_write(
        ...
        (s.project.colors.burnt_orange, "If work period for a presentation is "),
    ...
    st_write(
        ...
        (s.project.colors.forest_green + s.bold, "(80/P)*N hours"),
        (s.project.colors.burnt_orange, " of work "),
    )
    st_write(s.project.pres.paragraphs.p_xl, "e.g. project last over 20 calendar weeksif first presentation at week 5 -> (80/20)x5=20 hours of work per student ")
    st_space(size=3)
    st_space(size=3)
    st_space(size=3)
    ...
    st_write(s.project.pres.paragraphs.p_xl, "3", "0h*student*project ")
    ...
    st_write(s.project.pres.paragraphs.p_xl, "= ")
    st_write(s.project.pres.paragraphs.p_xl, "8-12 HQ pages * student ")
    st_space(size=3)
    st_space(size=3)
    st_space(size=3)
    ...
    st_write(
        ...
        (s.project.colors.burnt_orange + s.bold, "AI for Entrepreneurship and Innovation"),
        ".",
    )
    st_space(size=3)
    st_write(
