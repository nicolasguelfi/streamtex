...
def build():
    st_space(size=3)
    st_write(s.project.pres.titles.h2 + s.project.colors.forest_green, "Practice - MNIST_GAN ", tag=t.h2, toc_lvl="+1")
    st_space(size=3)
    ...
    with st_list(list_type=lt.ordered, li_style=s.project.pres.lists.item) as lst:
        with lst.item():
            st_write(" Inspect how GAN can be applied to MNIST ")
        with lst.item():
            st_write(" Check the performance of GAN for Digits Recognition ")
