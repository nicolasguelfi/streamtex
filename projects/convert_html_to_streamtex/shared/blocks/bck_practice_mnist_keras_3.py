...
def build():
    st_space(size=3)
    st_space(size=3)
    st_write(s.project.pres.titles.h2 + s.project.colors.forest_green, "Practice - MNIST_Keras 3 ", tag=t.h2, toc_lvl="+1")
    st_space(size=3)
    st_write(s.project.pres.titles.h4 + s.project.colors.burnt_orange, "Goal(s) ", tag=t.h4)
    st_write(
        s.project.pres.paragraphs.p_xl,
        (s.project.colors.olive_green + s.bold, "Increase the understanding"),
        " level of the MNIST Deep learning notebook content(Conceptually and Technically)  ",
    )
    st_space(size=3)
    ...
    with st_list(list_type=lt.ordered, li_style=s.project.pres.lists.item) as lst:
        ...
        with lst.item():
            st_write(s.project.colors.bright_red + s.bold, " CLICK INSIDE THE CELL BELOW THIS ONE:")
        with lst.item():
            st_write(
                s.project.pres.paragraphs.p_xl,
                " run all above cells",
