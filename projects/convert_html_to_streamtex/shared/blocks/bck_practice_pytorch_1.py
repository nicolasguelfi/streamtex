...
def build():
    ...
    st_write(
        ...
        (s.project.colors.burnt_orange, "PRACTICE PyTorch "),
        (s.project.colors.salmon, "- "),
        (s.project.colors.salmon, "1"),
        (s.project.colors.salmon, " -"),
        tag=t.h2,
        toc_lvl="+1",
    )
    ...
    st_space(size=3)
    st_write(s.project.pres.paragraphs.p_xl + s.project.colors.olive_green + s.bold, "Experiment the notions learned ")
    st_write(s.project.pres.paragraphs.p_xl + s.project.colors.olive_green + s.bold, "Consult the related notion notebooks")
    st_space(size=3)
    st_space(size=3)
    ...
    with st_list(list_type=lt.ordered, li_style=s.project.pres.lists.item) as lst:
        with lst.item():
            st_write(
                s.project.pres.paragraphs.p_xl,
                " Open the notebook",
                (s.project.colors.gold + s.bold, "_MNIST_pytorch.ipynb"),
        ...
        with lst.item():
            st_write(
                ...
                (s.project.colors.burnt_orange + s.bold, "200"),
