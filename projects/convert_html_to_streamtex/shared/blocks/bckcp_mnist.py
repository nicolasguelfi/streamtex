...
def build():
    st_space(size=1)
    st_write(s.project.doc.titles.h2, "9.2. MNIST Handwritten Digits ", tag=t.h2, toc_lvl="+1")
    st_space(size=1)
    st_space(size=1)
    with st_grid(cols=2, grid_style=s.project.doc.grids.gap) as g:
        ...
        with g.cell():
            st_write(s.project.doc.tables.cell, "Terminology ")
        with g.cell():
            st_write(s.project.doc.tables.cell + s.project.colors.link_blue + s.bold, "link", link="https://developers.google.com/machine-learning/glossary#mnist")
        with g.cell():
