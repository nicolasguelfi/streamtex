...
def build():
    ...
    st_space(size=2)
    st_write(s.project.doc.paragraphs.p_lg + s.project.colors.gold + s.bold, "AIAI")
    st_write(s.project.doc.paragraphs.p_lg + s.project.colors.olive_green + s.bold, "COURSEPACK ")
    st_space(size=1)
    st_write(s.project.doc.paragraphs.p_lg + s.project.colors.gold + s.bold, "Artificial Intelligence -  ")
    st_write(s.project.doc.paragraphs.p_lg + s.project.colors.gold + s.bold, "Applied Introduction")
    st_write(s.project.doc.paragraphs.p_lg + s.project.colors.burnt_orange + s.italic, "Concepts, Applications and Challenges")
    st_space(size=1)
    st_space(size=1)
