...
def build():
    ...
    st_write(
        ...
        "as sx.set_static_sources(), sx.get_static_sources(), and sx.resolve_static(). "
...
""")
    ...
    show_details(
        ...
        "You can retrieve the current configuration with sx.get_static_sources()."
    )
    st_space("v", 3)
...
""")
    ...
    show_details(
        ...
        "The function is deterministic: given the same sources and filename, it always "
...
# Used in test_project_advanced/book.py
...
shared_blocks = sx.LazyBlockRegistry([shared_blocks_path])
...
result = sx.resolve_static("nonexistent.json")
...
""")
    ...
    show_details(
        ...
        "4. Use sx.get_static_sources() to print the current source list."
    )
    st_space("v", 3)
    ...
    with st_block(bs.tip_box):
        ...
        st_write(
            s.medium,
            "For data files (JSON, CSV, text), use sx.resolve_static() explicitly. "
    ...
    with st_block(bs.tip_box):
        ...
        st_write(
            s.medium,
            "When assets are not found, print sx.get_static_sources() to verify "
