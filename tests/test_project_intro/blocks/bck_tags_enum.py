...
def build():
    ...
    st_write(s.titles.page_title, "Semantic Tags (HTML Markup)", tag=t.h1, toc_lvl="1")
    st_space("v", 1)
...
Tags also control table of contents generation in StreamTeX.
    ...
    # HEADING TAGS
    # ========================================================================
    st_write(s.titles.section_title, "Heading Tags (h1-h6)", toc_lvl="+1")
    st_space("v", 1)
...
In StreamTeX, heading tags automatically update the table of contents.
    ...
    st_write(s.titles.feature_title, "Available heading tags:")
    show_code("""
...
st_write(s.body, "Subsection", tag=t.h3)
# ... h4, h5, h6 also available
    """, language="python")

    st_write(s.titles.feature_title, "With Table of Contents:")
    show_code("""
...
st_write(s.body, "Subsection", tag=t.h3, toc_lvl="+1")
    """, language="python")
    ...
    # BLOCK TAGS
    # ========================================================================
    st_write(s.titles.section_title, "Block Tags (paragraph, div)", toc_lvl="+1")
    st_space("v", 1)
...
div = generic container (can hold multiple elements)
    ...
    st_write(s.titles.feature_title, "Usage:")
    show_code("""
from streamtex.enums import Tags as t

# Single paragraph
st_write(s.body, "This is a paragraph.", tag=t.p)

# Generic container (usually implicit in st_block)
st_write(s.body, "Container content", tag=t.div)
    """, language="python")
    ...
    # INLINE TAGS
    # ========================================================================
    st_write(s.titles.section_title, "Inline Tags (span)", toc_lvl="+1")
    st_space("v", 1)
...
Used with tuple syntax in st_write() for mixed-style text.
    ...
    st_write(s.titles.feature_title, "Usage with mixed styles:")
    show_code("""
from streamtex.enums import Tags as t
...
st_write(s.body,
    ...
    # ALL AVAILABLE TAGS
    # ========================================================================
    st_write(s.titles.section_title, "Complete Tag Reference", toc_lvl="+1")
    st_space("v", 1)
...
- None: No semantic tag (default)
    ...
    # ========================================================================
    st_write(s.titles.section_title, "Tags + Table of Contents", toc_lvl="+1")
    st_space("v", 1)
...
StreamTeX automatically numbers and links headings.
    ...
    st_write(s.titles.feature_title, "Complete example:")
    show_code("""
...
st_write(s.body, "Content here...", tag=t.p)

st_write(s.subtitle, "Section 1.2", tag=t.h2, toc_lvl="=1")
st_write(s.body, "More content...", tag=t.p)
...
st_toc(toc_config)
    ...
    # SEMANTIC MEANING
    # ========================================================================
    st_write(s.titles.section_title, "Why Semantic Tags Matter", toc_lvl="+1")
    st_space("v", 1)
...
- Use span for inline styling only
    ...
    # COMMON MISTAKES
    # ========================================================================
    st_write(s.titles.section_title, "Common Mistakes to Avoid", toc_lvl="+1")
    st_space("v", 1)
